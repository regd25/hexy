...
import { SOLArtifact, SOLArtifactType } from "../artifacts"
...
export class InMemoryArtifactRepository implements ArtifactRepository {
  ...
  async delete(id: string): Promise<boolean> {
    ...
    if (dependents.length > 0) {
      throw new Error(
        `Cannot delete artifact ${id}: has dependents ${dependents
          .map((d) => d.metadata.id)
          .join(", ")}`
      )
    }
  ...
  async findDependents(artifactId: string): Promise<SOLArtifact[]> {
    ...
    for (const artifact of this.artifacts.values()) {
      ...
      if (
        artifact.relationships?.supports?.includes(targetRef) ||
  ...
  async findByAuthor(author: string): Promise<SOLArtifact[]> {
    return Array.from(this.artifacts.values()).filter(
      (artifact) => artifact.metadata.author === author
    )
  }
  ...
  async findByComposition(
    ...
    return Array.from(this.artifacts.values()).filter((artifact) => {
      ...
      return Object.entries(uses).every(
        ([key, value]) =>
  ...
  async findByOrganizationalLevel(
    level: "strategic" | "tactical" | "operational"
  ): Promise<SOLArtifact[]> {
    return Array.from(this.artifacts.values()).filter(
      (artifact) => artifact.organizational?.level === level
    )
  }

  async findByArea(area: string): Promise<SOLArtifact[]> {
    return Array.from(this.artifacts.values()).filter(
      (artifact) => artifact.organizational?.area === area
    )
  }

  async findExecutableArtifacts(): Promise<SOLArtifact[]> {
    return Array.from(this.artifacts.values()).filter(
      (artifact) => artifact.isExecutable === true
    )
  }

  async findInvalidArtifacts(): Promise<SOLArtifact[]> {
    return Array.from(this.artifacts.values()).filter(
      (artifact) => artifact.isValid === false
    )
  }
  ...
  async findByValidationStatus(isValid: boolean): Promise<SOLArtifact[]> {
    return Array.from(this.artifacts.values()).filter(
      (artifact) => artifact.isValid === isValid
    )
  }
  ...
  async search(query: ArtifactSearchQuery): Promise<ArtifactSearchResult> {
    ...
    if (query.organizationalLevel) {
      results = results.filter(
        (a) => a.organizational?.level === query.organizationalLevel
      )
    }
  ...
  async getStatistics(): Promise<RepositoryStatistics> {
    ...
    const recentlyModified = artifacts.filter(
      (a) => a.metadata.lastModified > weekAgo
    ).length

    const validationScores = Array.from(this.validationResults.values()).map(
      (r) => r.score
    )
  ...
  private generateFacets(artifacts: SOLArtifact[]): SearchFacets {
    ...
    for (const artifact of artifacts) {
      ...
      const validationCount =
        validationCounts.get(artifact.isValid || false) || 0
    ...
    return {
      ...
      authors: Array.from(authorCounts.entries()).map(([author, count]) => ({
        author,
        count,
      })),
      areas: Array.from(areaCounts.entries()).map(([area, count]) => ({
        area,
        count,
      })),
      validationStatus: Array.from(validationCounts.entries()).map(
  ...
  async findByReferences(
    references: string[]
  ): Promise<Map<string, SOLArtifact>> {
