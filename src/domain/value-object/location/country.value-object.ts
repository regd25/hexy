...
/**
 * @description A value object that represents a country.
 * @example
 * const country = new Country('MX')
 */
export class Country extends ValueObject<keyof typeof Country.Country> {
	...
	/**
	 * @description A record of country data.
	 * @example
	 * const countryData = Country.COUNTRY_DATA
	 */
	...
	constructor(value: keyof typeof Country.Country) {
		...
		if (!this.validate(value)) {
			throw new InvalidLocation(Country, { country: value })
		}
	}

	validate(value: keyof typeof Country.Country): boolean {
