import { Repository, EventBus } from 'shared/contracts'
import type { AggregateRoot, IdentifierValueObject } from 'shared/core'
...
export class InMemoryRepository<T extends AggregateRoot> extends Repository<T> {
	...
	async matching(criteria: Criteria): Promise<T[]> {
		// Simple implementation that filters in-memory
		// In a real implementation, this would be delegated to a criteria converter
		return Array.from(this.items.values()).filter((item) => {
			// Basic implementation - in a real scenario, this would use a proper criteria matcher
			const primitives = item.toPrimitives()

			// Check if all filter conditions match
			for (const [field, value] of Object.entries(
				criteria.toPrimitive().filters || {},
			)) {
