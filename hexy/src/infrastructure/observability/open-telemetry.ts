...
import { Span, Telemetry, TelemetryOptions } from './telemetry'

class OpenTelemetrySpan implements Span {
  constructor(private readonly name: string) {}

  addAttribute(key: string, value: string | number | boolean): void {
    console.log(`[OpenTelemetry] Adding attribute ${key}=${value} to span ${this.name}`)
  }
...
export class OpenTelemetry implements Telemetry {
  constructor(private readonly options: TelemetryOptions) {
    console.log(`[OpenTelemetry] Initializing with service ${options.serviceName} v${options.serviceVersion}`)
  }
