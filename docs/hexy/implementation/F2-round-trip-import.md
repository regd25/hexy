...
## 1. Objetivo
Cerrar el round-trip: **importar un `.sop`** (el que F1 exportó, o un template de
`docs/sol/templates/`) → parsear → reconstruir el grafo de artefactos → renderizar y validar
al cargar. Demuestra que `.sop` es un formato de intercambio real, no solo de salida.

## 2. Alcance E2E
- **Parser (`@hexy/sol`):** `parse(sop: string)` → objeto SOL (YAML) → `VisualArtifact[]` +
  `Relationship[]` (resolver `uses:` y referencias `Type:Id` a aristas del grafo).
- **UI:** acción **Import `.sop`** (file picker / drag-drop) que carga el grafo en el canvas.
...
El usuario importa el `.sop` exportado en F1 (o `intent-template.sop`) y **ve el mismo grafo**
